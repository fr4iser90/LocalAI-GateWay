...
class RateLimiter:
    """In-memory RPM + concurrency + daily counters (key / team / model scopes)."""
    ...
    def _rpm_check(self, bucket: str, rpm: int | None, now: float) -> tuple[bool, int | None, int | None]:
        ...
        with self._lock:
            ...
            if model_bucket and model_concurrency and model_concurrency > 0:
                if self._inflight[model_bucket] >= model_concurrency:
                    return LimitDecision(False, "model_concurrency_exceeded", remaining, 1)
    ...
    def release(self, key_id: int, model: str | None = None) -> None:
        key_bucket = f"key:{key_id}"
        model_bucket = f"key:{key_id}:model:{model}" if model else None
        with self._lock:
    ...
    def quota_usage_pct(
        self, *, key_id: int, team_id: int | None, key_q: int | None, team_q: int | None
    ) -> float | None:
        today = _utc_today()
        with self._lock:
            pcts = []
