...
from ..data.models import WebUser, AuthSettings, PasswordResetToken, SmtpConfig, Team, TeamMember, utcnow
from .session import Forbidden, require_platform_admin, require_user, current_user
...
def create_reset_token(
    db: Session, user: WebUser, *, by_platform_admin: bool = False
) -> str:
    ...
    db.add(
        PasswordResetToken(
            ...
            created_by_platform_admin=by_platform_admin,
        )
    )
    return raw
...
def find_user_by_login(db: Session, login: str) -> WebUser | None:
    ...
    if "@" in login:
        return (
            db.query(WebUser)
            .filter(WebUser.email == login.lower())
            .first()
        )
    return db.query(WebUser).filter(WebUser.username == login).first()
...
def max_keys_allowed(db: Session, owner: WebUser | None) -> int | None:
...
def assert_can_create_key(db: Session, owner: WebUser | None) -> str | None:
...
def forgot_submit(
    ...
    if user and user.is_active and user.email:
        try:
            raw = create_reset_token(db, user, by_platform_admin=False)
...
def reset_submit(
    ...
    user = db.get(WebUser, row.user_id)
    ...
    *,
    ...
    if email_changed:
        if new_email:
            other = (
                db.query(WebUser)
                .filter(WebUser.email == new_email, WebUser.id != user.id)
                .first()
            )
            if other:
...
async def smtp_save(
    ...
    cfg.from_name = str(form.get("from_name") or "OnPrem AI Gateway").strip()
...
def smtp_test(
    ...
    try:
        send_mail(
            db,
            to_email=to,
            subject="OnPrem AI Gateway SMTP test",
            body_text=f"SMTP test OK from OnPrem AI Gateway (sent by {user.username}).\n",
        )
